...
start "EduPulse Backend" cmd /k "cd /d e:\UNI Important\5th semester\Machine Learning\backend && echo Installing backend dependencies... && pip install -r requirements.txt -q && echo. && echo Starting FastAPI server on http://localhost:8000 && echo. && python -m uvicorn main:app --reload --host 0.0.0.0 --port 8000"
